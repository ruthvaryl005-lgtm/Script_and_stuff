...
def make_frame(t):
    surface = gizeh.Surface(W, H, bg_color=(0, 0, 0))  # black background
    ...
    for i in range(num_arms):
        ...
        # Draw arm
        arm = gizeh.rectangle(lx=arm_length*2, ly=20,
                               xy=(center[0]+arm_length/2, center[1]),
                               fill=arm_color, angle=angle)
        arm.draw(surface)

        # Draw joint circle
        joint_circle = gizeh.circle(r=10 + sin(t*3)*3, xy=arm_tip_pos, fill=arm_color)
